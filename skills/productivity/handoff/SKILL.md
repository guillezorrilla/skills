...
Do not duplicate content already captured in other artifacts (specs, plans, ADRs, issues, commits, diffs). Reference them by path or URL instead. If `docs/agents/tracker.md` and `docs/agents/forge.md` exist, use them to build real links — the ticket base URL and the repo slug are recorded there — so the next agent gets a URL it can open rather than a bare key.
